...
ATR2* create_atr2() 
{
  ...
  if(atr2p == NULL) exit(55);

   atr2p->occupied = 0;

   for (int i = 0; i < MAX_ATR2; i++)
   {
      atr2p->atr2[i] = NO_TYPE;
   }
   return atr2p;
}
...
int insert_literal(char *str, unsigned type) {
  ...
  idx = insert_symbol(str, LIT, type, NO_ATR, create_atr2());
  return idx;
}
...
void print_symtab(void) {
  ...
  for(i = 0; i < first_empty; i++) {
    ...
    // atr2 printing
    printf("\t\tatr2(list of them):\n");
    printf("\t\t-----\n");
    
    for (int j = 0; j < symbol_table[i].atr2->occupied; j++)
    {
...
void init_symtab(void) {
  ...
  for(i = 0; i < 14; i++) {
    sprintf(s, "%%%d", i);
    insert_symbol(strdup(s), REG, NO_TYPE, NO_ATR, create_atr2());
  }
}
